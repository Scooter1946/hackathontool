...
export function renderTeamctxShell(o: ArtifactOptions): string {
  return [
    ...
    "",
    "# One-time GitHub setup (only when the repo is on GitHub): authenticate as yourself so your",
    "# commits and pushes are attributed to your own account. teamctx never sees the token — gh",
    "# stores it in your own 0700 home, exactly like the Claude login below.",
    'if git -C "${REPO}" remote get-url origin 2>/dev/null | grep -qi github.com; then',
    "  if ! gh auth status >/dev/null 2>&1; then",
    '    echo "── teamctx: connect your GitHub account (one-time) ──"',
    '    echo "  gh will show a URL + one-time code — open it on your laptop to authorize."',
